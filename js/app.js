...
// ─── Program import ─────────────────────────────────────────────────────────
// Cycles are generated in Claude Code (against the rulebook in programGen.js)
// and pasted in here as JSON, rather than generated by an in-app API call.
function handleImport() {
  const raw = $('import-json').value.trim();
  if (!raw) { toast('Paste the program JSON first', 'error'); return; }

  const comments = $('previous-comments').value.trim();
  if (comments) Storage.saveLastComments(comments);

  let program;
  try {
    program = ProgramGen.parseProgramJSON(raw);
  } catch (err) {
    console.error(err);
    showError(`Could not import program: ${err.message}`);
    return;
  }
